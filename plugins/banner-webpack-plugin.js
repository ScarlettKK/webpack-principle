...
class BannerWebpackPlugin {
    ...
    apply(compiler) {
        ...
        // 压缩之后冻结之前处理最佳，不过如果在compilation的hook中处理，下面的方法可能会被触发多次，只在compiler的hook中就只处理一次了
        compiler.hooks.emit.tapAsync("BannerWebpackPlugin", (compilation, callback) => {
            ...
            assetPaths.forEach((assetPath) => {
                // compilation.assets + 资源路径 得到资源对象，.source()方法得到具体内容
                const source = compilation.assets[assetPath].source();

                // 拼上注释
                // options带有用户传入参数
                // newSource也可以写成newSource = prefix + source，其中prefix是注释部分，注意换行部分
                const newSource = `/*
...
${source}`;
                ...
                compilation.assets[assetPath] = {
                    // 最终资源输出时，会调用source方法，最终source方法的返回值就是资源的具体内容
                    // 覆盖原来资源内容
                    source() {
                        return newSource;
                    },
